...
function Main(){
  const preferencesValue=_c_1.Create_1("");
  const preferenceSavedValue=_c_1.Create_1("");
  const preferencesOutput=_c_1.Create_1("");
  const clipboardValue=_c_1.Create_1("");
  const writeClipboardOutput=_c_1.Create_1("");
  const cameraOutput=_c_1.Create_1("");
  const logInOutput=_c_1.Create_1("");
  const t=new _c_3("New_1");
  const this_1=(t.h.push(EventQ2(t.k, "takephoto", () => t.i, () => {
    const _3=null;
    Start(Delay(() => AsAsync(takePicture().then((image) => _c_1.Set(cameraOutput, String(Substring(image.base64String, 0, 20)))))), null);
  })),t);
  const this_2=(this_1.h.push(new _c_8("cameraoutput", cameraOutput.View)),this_1);
  const t_1=(this_2.h.push(new _c_9("clipboardvalue", clipboardValue)),this_2);
  const this_3=(t_1.h.push(EventQ2(t_1.k, "writeclipboard", () => t_1.i, () => {
    const _3=null;
    Start(Delay(() => AsAsync(writeToClipboard(clipboardValue.Get()).then(() => _c_1.Set(writeClipboardOutput, "Successfully write '"+String(clipboardValue.Get())+"' to clipboard")))), null);
  })),t_1);
  const t_2=(this_3.h.push(new _c_8("writeclipboardoutput", writeClipboardOutput.View)),this_3);
  const t_3=(t_2.h.push(EventQ2(t_2.k, "setcookies", () => t_2.i, () => {
    const _3=null;
    Start(Delay(() => AsAsync(setCookies().then(() => {
      const _4=(s) => {
        console.log(s);
      };
      return _4("Successfully Set Cookies");
    }))), null);
  })),t_2);
  const t_4=(t_3.h.push(EventQ2(t_3.k, "showalert", () => t_3.i, () => {
    const _3=null;
    Start(Delay(() => AsAsync(showAlert().then(() => {
      const _4=(s) => {
        console.log(s);
      };
      return _4("Successfully Show Alert");
    }))), null);
  })),t_3);
  const this_4=(t_4.h.push(EventQ2(t_4.k, "showconfirm", () => t_4.i, () => {
    const _3=null;
    Start(Delay(() => AsAsync(showConfirm().then(() => {
      const _4=(s) => {
        console.log(s);
      };
      return _4("Successfully Show Confirm");
    }))), null);
  })),t_4);
  const t_5=(this_4.h.push(new _c_9("preferencesvalue", preferencesValue)),this_4);
  const this_5=(t_5.h.push(EventQ2(t_5.k, "save", () => t_5.i, () => {
    const _3=null;
    Start(Delay(() => AsAsync(save(preferencesValue.Get()).then(() => _c_1.Set(preferenceSavedValue, "Save '"+String(preferencesValue.Get())+"' Successfully")))), null);
  })),t_5);
  const t_6=(this_5.h.push(new _c_8("preferencessavedvalue", preferenceSavedValue.View)),this_5);
  const this_6=(t_6.h.push(EventQ2(t_6.k, "get", () => t_6.i, () => {
    const _3=null;
    Start(Delay(() => AsAsync(get_1().then((token) => _c_1.Set(preferencesOutput, "Value: "+String(token.value))))), null);
  })),t_6);
  const t_7=(this_6.h.push(new _c_8("preferencesoutput", preferencesOutput.View)),this_6);
  const this_7=(t_7.h.push(EventQ2(t_7.k, "login", () => t_7.i, () => {
    const _3=null;
    Start(Delay(() => AsAsync(login().then(() => _c_1.Set(logInOutput, "Finger print log in")))), null);
  })),t_7);
  const t_8=(this_7.h.push(new _c_8("loginoutput", logInOutput.View)),this_7);
  const t_9=(t_8.h.push(EventQ2(t_8.k, "notification", () => t_8.i, () => {
    const _3=null;
    Start(Delay(() => AsAsync(scheduleNotification().then(() => {
      const _4=(s) => {
        console.log(s);
      };
      return _4("Scheduled notification complete");
    }))), null);
  })),t_8);
  const b=(t_9.h.push(EventQ2(t_9.k, "deliverednotification", () => t_9.i, () => {
    const _3=null;
    Start(Delay(() => AsAsync(getDeliveredNotification().then((notification) => alert(JSON.stringify(notification))))), null);
  })),t_9);
  const p=CompleteHoles(b.k, b.h, [["clipboardvalue", 0, null], ["preferencesvalue", 0, null]]);
...
function CompleteHoles(key, filledHoles, vars){
  ...
  return[append(filledHoles, choose((_1) => {
    ...
    if(filledVars.Contains(name))return null;
      ...
      }):ty===4?_c_14.GetOrAddHoleFor(key, name, () => new _c_18(name, _c_1.Create_1([]))):ty===5?_c_14.GetOrAddHoleFor(key, name, () => new _c_19(name, _c_1.Create_1(Some(globalThis.document.querySelector("[ws-dom="+name+"]"))))):ty===6?_c_14.GetOrAddHoleFor(key, name, () => {
        const o=d==null?null:Some(d.$0);
        let _2=_c_1.Create_1(o==null?[]:o.$0);
...
function takePicture(){
  ...
  return new Promise((_2) => {
    _2(Camera.getPhoto({resultType:"base64"}).then((d) => Promise.resolve(d)));
  });
}
function writeToClipboard(value){
  ...
  return new Promise((_2) => {
    let r;
    _2(Clipboard.write((r={},r.string=String(value),r)).then(() => Promise.resolve(null)));
  });
}
function setCookies(){
  const _1=null;
  return new Promise((_2) => {
    let r;
    _2(CapacitorCookies.setCookie((r={},r.url="http://example.com'",r.key="language",r.value="en",r)).then(() => Promise.resolve(null)));
  });
}
function showAlert(){
  const _1=null;
  return new Promise((_2) => {
    let r;
    _2(Dialog.alert((r={},r.title="Stop",r.message="this is an error",r)).then(() => Promise.resolve(null)));
  });
}
function showConfirm(){
  ...
  return new Promise((_2) => {
    let r;
    _2(Dialog.confirm((r={},r.title="Confirm",r.message="Are you sure you'd like to press the red button?",r)).then((d) => Promise.resolve(d)));
  });
}
function save(value){
  const _1=null;
  return new Promise((_2) => {
    _2(Preferences.set({key:"auth_token", value:value}).then(() => Promise.resolve(null)));
  });
}
function get_1(){
  const _1=null;
  return new Promise((_2) => {
    _2(Preferences.get({key:"auth_token"}).then((d) => Promise.resolve(d)));
  });
}
function login(){
  const _1=null;
  return new Promise((_2) => {
    let r;
    _2((BiometricAuth.authenticate((r={},r.reason="Please authenticate",r.cancelTitle="Cancel",r.allowDeviceCredential=true,r.iosFallbackTitle="Use passcode",r.androidTitle="Biometric login",r.androidSubtitle="Log in using biometric authentication",r.androidConfirmationRequired=false,r.androidBiometryStrength="weak",r)),Promise.resolve(null)));
  });
}
function scheduleNotification(){
  ...
  return new Promise((_2) => {
    let r;
    let r_1;
    _2(LocalNotifications.schedule((r={},r.notifications=[(r_1={},r_1.id=1,r_1.title="Reminder Notification",r_1.body="Explore new variety and offers",r_1.largeBody="Get 30% discounts on new products",r_1.summaryText="Exciting offers",r_1)],r)).then((d) => Promise.resolve(d)));
  });
}
function getDeliveredNotification(){
  const _1=null;
  return new Promise((_2) => {
    _2(LocalNotifications.getDeliveredNotifications().then((d) => Promise.resolve(d)));
  });
}
...
function unwrapExn(x){
  return x instanceof Error?x:new _c_23(x);
}
...
function InlineTemplate(el, fillWith){
  ...
  foreachNotPreserved(el, "[ws-on]", (e_1) => {
    addAttr(e_1, _c_37.Concat(choose((x_1) => {
      ...
      if(m[0]){
        const th=m[1];
        return th instanceof _c_36?Some(Handler(get(a, 0), th.Value)):th instanceof _c_10?Some(_c_37.Handler(get(a, 0), th.Value)):(console.warn("Event hole on"+get(a, 0)+" filled with non-event data", get(a, 1)),null);
      }
      else return null;
...
let _c_14=Lazy((_i) => class TemplateInitializer extends _c {
  ...
  static {
    _c_14.initialized=new _c_12("New_5");
    _c_14.instances=new _c_12("New_5");
  }
});
let _c_15=Lazy((_i) => {
  Force(_c_4);
...
let _c_21=Lazy((_i) => class Exception extends _c {
  static {
    _c_21=_i(this);
  }
});
...
let _c_24=Lazy((_i) => class $StartupCode_Concurrency {
  static {
    _c_24=_i(this);
  }
  static GetCT;
...
function Dynamic(view, set_1){
  return _c_37.A1(new _c_46(view, set_1));
}
function EmptyAttr(){
...
let _c_46=Lazy((_i) => class DynamicAttrNode extends _c {
  static {
    _c_46=_i(this);
  }
  push;
...
let _c_48=Lazy((_i) => class Client {
  ...
  static {
    ...
    this.IntGetChecked=(el) => {
      ...
      let o;
      const s_8=el.value;
      if(isBlank(s_8))_1=(el.checkValidity?el.checkValidity():true)?_c_50.Blank(s_8):_c_50.Invalid(s_8);
      else {
        const m=(o=0,[TryParse(s_8, {get:() => o, set:(v) => {
          o=v;
        }}), o]);
        _1=m[0]?_c_50.Valid(m[1], s_8):_c_50.Invalid(s_8);
      }
      return Some(_1);
    };
    ...
    this.FloatGetChecked=(el) => {
      ...
      if(isBlank(s_8))_1=(el.checkValidity?el.checkValidity():true)?_c_50.Blank(s_8):_c_50.Invalid(s_8);
        ...
        const i=+s_8;
        _1=isNaN(i)?_c_50.Invalid(s_8):_c_50.Valid(i, s_8);
      }
      return Some(_1);
    };
...
function ApplyValue(get_2, set_1, var_1){
  let expectedValue;
  expectedValue=null;
  return[(el) => {
    const onChange=() => {
...
function FileApplyValue(get_2, set_1, var_1){
  let expectedValue;
  expectedValue=null;
  return[(el) => {
...
let _c_50=Lazy((_i) => class CheckedInput {
  static {
    _c_50=_i(this);
  }
  get Input(){
